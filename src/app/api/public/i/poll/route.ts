...
export async function POST(request: Request) {
  ...
  if (!invitation) {
    return corsResponse({ status: "error", error: "Invalid invitation" });
  }
  ...
  if (data.error === "authorization_pending") {
    return corsResponse({ status: "pending" });
  }
  ...
  try {
    ...
    if (!profileResponse.ok) {
      return corsResponse({ status: "error", error: "Failed to get profile" });
    }
    ...
    // Webhook subscription (non-critical)
    try {
